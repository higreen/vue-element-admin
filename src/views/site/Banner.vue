<template>
  <main>
    <el-header class="text-r">
      ...
      >
        <el-button type="primary" @click="list_index=null">添加横幅</el-button>
      </el-upload>
    </el-header>
    <ul class="list">
...
export default {
  ...
  methods: {
    ...
    saveInfo(item) {
      if (item.id) {
        ajax.put(`/admin/banners/${item.id}`, item)
          .then(() => {
            this.$message.success('保存成功')
          })
      } else {
        ajax.post('/admin/banners', item)
          .then(() => {
